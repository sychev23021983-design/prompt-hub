...
def load_existing_progress(conn):
    ...
    structure_node_id, page_type). url, structure_node_id и page_type тоже
    переносятся при переимпорте, чтобы вставленный вручную URL, привязка к
    разделу структуры и выбранный тип страницы (для промпт-шаблонов) не
    терялись, когда xlsx обновляется свежей выгрузкой из KeyCollector.
    ...
    rows = conn.execute("""
        SELECT s.slug, r.sheet, r.url, r.name, r.applied, r.date_applied, r.notes,
               r.structure_node_id, r.page_type
...
def import_shustrik_maps(conn, site_id, path, progress):
    ...
    for row in cell_map(wb["Products_SEO"]):
        ...
        applied, date_applied, notes, url, node_id, page_type = progress.get(
            key, (row.get("Applied?", "No"), row.get("Date Applied"), row.get("Notes"), row.get("URL"), None, None))
        conn.execute("""INSERT INTO rows_
            (site_id, sheet, name, url, row_type, seo_title, h1, meta_description,
             ...
             applied, date_applied, notes, structure_node_id, page_type)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
...
def import_fit(conn, site_id, path, progress):
    ...
    for row in cell_map(wb["Pages_SEO"]):
        ...
        applied, date_applied, notes, url, node_id, page_type = progress.get(
            key, (row.get("Applied?", "No"), row.get("Date Applied"), row.get("Notes"), None, None, None))
        conn.execute("""INSERT INTO rows_
            (site_id, sheet, name, url, row_type, seo_title, h1, meta_description,
             primary_keyword, secondary_keywords, lsi_keywords, faq_questions, extra_json,
             applied, date_applied, notes, structure_node_id, page_type)
